...
import { getVisualSimilarProducts } from "../api/recommendationService";
...
export function VisualSimilarProducts({
  productId,
  currentProductName,
}: VisualSimilarProductsProps) {
  const { state } = useApp();
  ...
  useEffect(() => {
    ...
    const fetchProducts = async () => {
      ...
      const result = await getVisualSimilarProducts(productId, 8);
  ...
  if (!products || products.length === 0) {
    return null; // Ukryj sekcję jeśli brak produktów
  }
    ...
    <div className="py-8">
      <div className="mb-6">
        ...
        <p className="text-gray-600 dark:text-gray-400">
          Produkty o podobnym wyglądzie do:{" "}
